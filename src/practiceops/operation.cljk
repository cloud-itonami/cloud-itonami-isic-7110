(ns practiceops.operation
  "OperationActor -- one architectural/engineering-practice operation
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (PracticeOps-LLM) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Design Governor
  ...
  One graph run = one practice operation (intake -> advise -> govern ->
  ...
  decision to a human operator (practice principal/licensed
  professional). The approver resumes with `{:approval {:status
  :approved}}` (or :rejected). `:design/verify`/`:design/deliver`
  ALWAYS reach this node when the governor is clean -- see
  `practiceops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [practiceops.practiceopsllm :as practiceopsllm]
...
(defn build
  ...
  `practiceops.store/Store`).
  opts:
    :advisor      -- a `practiceops.practiceopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (practiceopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PracticeOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (practiceopsllm/-advise advisor store request)]
            {:proposal p :audit [(practiceopsllm/trace request p)]})))

      ;; Design Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
